...
import {
  AnimatedSection,
  HeroCarousel,
} from "@/components/HomeClientComponents";
import { Music, PlusCircle, Sparkles } from "lucide-react";
import Link from "next/link";

export default async function Home() {
  const [featuredSongs, popularSongs, recentSongs] = await Promise.all([
    getFeaturedHeroSongs(),
    getTrendingSongs(8),
    getLatestSongs(8),
  ]);

  const heroSongs = featuredSongs.length > 0
    ? featuredSongs
    : popularSongs.slice(0, 3);
          ...
          : (
            <div className="relative w-full h-[35vh] min-h-[280px] bg-gradient-to-b from-slate-900/80 to-black border-b border-white/10 flex flex-col items-center justify-center text-center p-6">
              ...
              <p className="text-slate-400 text-sm max-w-md mt-1">
                Platform Chord Gitar AI Terlengkap dengan Transkripsi Real-time.
              </p>
            </div>
          )}
          ...
          ? (
            <div className="px-4 md:px-8 lg:px-12 py-16 text-center flex flex-col items-center justify-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">
                Belum Ada Lagu di Database
              </h3>
              <p className="text-slate-400 max-w-md mb-6 text-sm">
                Mulai tambahkan lagu favoritmu atau generate chord otomatis
                dengan AI Generator sekarang!
              </p>
              <Link
                href="/ai-generator"
