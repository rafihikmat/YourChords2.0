...
import { Play, Pause, User, Music, ArrowLeft, FileText, Minus, Plus, Type, Mic2, Anchor, Printer, SearchX, Book, Gauge, ChevronLeft, ChevronRight, X } from 'lucide-react';
...
export default function SongDetail() {
    ...
    // Data Fetching
    const { song, loading, error } = useSongData(id);

    // View State
    const { isAutoScrolling, setIsAutoScrolling, scrollSpeed, setScrollSpeed } = useAutoScroll();
    ...
    // Reset variation index when chord changes
    useEffect(() => {
        setVariationIndex(0);
    }, [selectedChord]);
    ...
    const chordVariations = useMemo(() => {
        ...
        return ChordAdapter.getAllChordVoicings(selectedChord) || [];
    }, [selectedChord]);
                ...
                {selectedChord && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={() => setSelectedChord(null)}>
                        <motion.div
                            ...
                            className="bg-slate-900 border border-white/10 rounded-2xl p-6 shadow-2xl flex flex-col items-center gap-6 max-w-sm w-full"
                            onClick={(e) => e.stopPropagation()}
                        >
                            <div className="flex justify-between items-center w-full border-b border-white/10 pb-4">
                                <div>
                                    <h3 className="text-2xl font-bold text-white font-mono">{selectedChord}</h3>
                                    <p className="text-xs text-slate-400">Variation {variationIndex + 1} of {chordVariations.length}</p>
                                </div>
                                <button onClick={() => setSelectedChord(null)} className="p-2 hover:bg-white/10 rounded-full text-slate-400 hover:text-white transition-colors">
                                    <X className="w-6 h-6" />
                                </button>
                            </div>
                            ...
                            <div className="flex items-center justify-between w-full gap-2">
                                <button
                                    onClick={() => setVariationIndex(i => (i - 1 + chordVariations.length) % chordVariations.length)}
                                    disabled={chordVariations.length <= 1}
                                    className="p-3 hover:bg-white/10 rounded-full disabled:opacity-20 disabled:cursor-not-allowed text-white transition-colors"
                                >
                                ...
                                <div className="bg-white/5 p-4 rounded-xl border border-white/5">
                                    <ChordDiagram
                                        name={selectedChord}
                                        position={chordVariations[variationIndex]}
                                ...
                                <button
                                    onClick={() => setVariationIndex(i => (i + 1) % chordVariations.length)}
                                    disabled={chordVariations.length <= 1}
                                    className="p-3 hover:bg-white/10 rounded-full disabled:opacity-20 disabled:cursor-not-allowed text-white transition-colors"
                                >
                            ...
                            <div className="text-center">
                                <span className="text-xs font-bold uppercase text-slate-500 tracking-wider">Starting Fret</span>
                                <p className="text-xl font-mono font-bold text-primary">
                                    {chordVariations[variationIndex]?.baseFret || 1}
                                </p>
                            </div>
                        </motion.div>
                    </div>
