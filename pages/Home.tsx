...
import { ArrowRight, Mic, Music2, Sparkles, Zap, Disc, Youtube } from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  const fallbackSongs: Song[] = [
    { id: '1', title: 'Neon Blade', artist: 'MoonDeity', chords: [], tablature: {}, difficulty: 'Expert', spotify_track_id: '123', youtube_video_id: '456', view_count: 125000 },
    { id: '2', title: 'After Dark', artist: 'Mr. Kitty', chords: [], tablature: {}, difficulty: 'Medium', spotify_track_id: '124', youtube_video_id: '457', view_count: 89000 },
    { id: '3', title: 'Nightcall', artist: 'Kavinsky', chords: [], tablature: {}, difficulty: 'Easy', spotify_track_id: '125', youtube_video_id: '458', view_count: 45000 },
    { id: '4', title: 'Resonance', artist: 'Home', chords: [], tablature: {}, difficulty: 'Medium', spotify_track_id: '126', youtube_video_id: '459', view_count: 32000 },
    { id: '5', title: 'Memory Reboot', artist: 'VØJ, Narvent', chords: [], tablature: {}, difficulty: 'Hard', spotify_track_id: '127', youtube_video_id: '460', view_count: 28000 },
    { id: '6', title: 'SimpsonWave 1995', artist: 'FrankJavCee', chords: [], tablature: {}, difficulty: 'Easy', spotify_track_id: '128', youtube_video_id: '461', view_count: 15000 }
  ];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch Songs
        const { data, error } = await supabase.from('songs').select('*').order('view_count', { ascending: false }).limit(6);
        if (error || !data || data.length === 0) setSongs(fallbackSongs);
        else setSongs(data as unknown as Song[]);
    ...
    <div className="relative w-full min-h-screen bg-slate-50 dark:bg-slate-950 overflow-hidden flex flex-col transition-colors duration-500">
      ...
      <div className="relative z-10 pt-36 pb-20 px-4 w-full flex flex-col items-center justify-center">
        ...
        >
            ...
            <h1 className="text-4xl md:text-7xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-slate-900 to-slate-500 dark:from-neutral-50 dark:to-neutral-400 tracking-tight mb-6">
                Master your chords <br /> in <span className="text-primary">Hyperspeed.</span>
            </h1>

            <p className="text-slate-600 dark:text-neutral-400 text-lg md:text-xl max-w-2xl mb-10">
                The most advanced guitar platform for the modern musician. 
                AI-generated chords, immersive tablature, and distraction-free practice modes.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button className="inline-flex h-12 items-center justify-center rounded-full border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 px-8 text-sm font-medium text-slate-900 dark:text-white transition-colors hover:bg-slate-100 dark:hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
                    Search Library
                </button>
            </div>
        </motion.div>
      ...
      <div className="relative z-10 px-6 pb-20 max-w-7xl mx-auto w-full">
      ...
      <div className="relative z-10 px-6 py-20 bg-slate-100 dark:bg-slate-900/50 border-t border-slate-200 dark:border-white/5 w-full">
         <div className="max-w-4xl mx-auto text-center mb-12">
             <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-4">Generate Chords with Gemini</h2>
             <p className="text-slate-600 dark:text-slate-400">Can't find a song? Just paste the lyrics, and our AI will transcribe it for you in seconds.</p>
         </div>
         <AIChordForm />
      </div>
