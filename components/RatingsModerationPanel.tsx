...
import { 
  Star, BarChart2, RefreshCw, Trash2, Search, CheckCircle2, Award, AlertCircle 
} from "lucide-react";
import { getAdminRatingsOverview, resetSongRatingsAndVotes, SongRatingModerationItem } from "@/lib/ratings";

interface RatingsModerationPanelProps {
  onRatingsReset?: () => void;
}

export default function RatingsModerationPanel({ onRatingsReset }: RatingsModerationPanelProps) {
  ...
  const handleReset = async (item: SongRatingModerationItem) => {
    const confirmAsk = window.confirm(
      `Apakah Anda yakin ingin mereset seluruh rating dan vote kesulitan lagu ini?`
    );
    if (!confirmAsk) return;

    setResettingId(item.song_id);
    try {
      const res = await resetSongRatingsAndVotes(item.song_id);
      if (res.success) {
        setToastMessage(`✨ Data rating & vote kesulitan berhasil di-reset ke nol!`);
        setTimeout(() => setToastMessage(null), 3500);

        // Optimistic UI update for this item in state
        setItems((prevItems) =>
          prevItems.map((i) =>
            i.song_id === item.song_id
              ? {
                  ...i,
                  averageRating: 0,
                  totalRatings: 0,
                  totalDifficultyVotes: 0,
                  difficultyBreakdown: { 'Sangat Mudah': 0, 'Mudah': 0, 'Sedang': 0, 'Sulit': 0 },
                }
              : i
          )
        );
        ...
        alert(res.error || "Gagal mereset data rating.");
      }
    } catch (err: any) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Kelola data ulasan bintang asli dan statistik vote kesulitan lagu dari pengguna.
            </p>
          </div>
        </div>
      ...
      <div className="overflow-x-auto">
        {loading ? (
          <div className="p-8 text-center text-slate-500 animate-pulse text-xs flex items-center justify-center gap-2">
            ...
            <span>Memuat data agregasi rating & vote kesulitan dari database...</span>
          </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-black/60 text-[10px] uppercase font-bold tracking-wider text-slate-400 border-b border-white/10">
              <tr>
                <th className="px-5 py-3">Judul Lagu & Artis</th>
                <th className="px-5 py-3 text-center">Rating Bintang</th>
                <th className="px-5 py-3 text-center">Tingkat Kesulitan</th>
                <th className="px-5 py-3 text-center">Tindakan Moderasi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/10">
              {filteredItems.map((item) => (
                <tr key={item.song_id} className="hover:bg-violet-500/5 transition-colors group">
                  <td className="px-5 py-3.5 font-bold text-white">
                    <div className="text-white group-hover:text-violet-300 transition-colors">{item.title}</div>
                    <div className="text-slate-400 text-[11px] font-normal">{item.artist}</div>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <div className="flex flex-col items-center">
                      <div className="flex items-center gap-1 font-mono font-extrabold text-amber-400 text-xs">
                        ...
                        <span>{item.totalRatings > 0 ? `${item.averageRating} / 5.0` : "Belum Ada"}</span>
                      </div>
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <div className="flex flex-col items-center gap-1">
                      <span className="px-2.5 py-0.5 bg-violet-500/20 text-violet-300 font-mono font-bold border border-violet-500/40 rounded-md text-[10px]">
                        {item.totalDifficultyVotes} Suara Total
                      </span>
                      {item.totalDifficultyVotes > 0 && (
                        <div className="flex items-center gap-1.5 text-[9px] font-mono text-slate-400">
                          <span className="text-emerald-400">SM: {item.difficultyBreakdown['Sangat Mudah']}</span>
                          <span>•</span>
                          <span className="text-blue-400">M: {item.difficultyBreakdown['Mudah']}</span>
                          <span>•</span>
                          <span className="text-amber-400">Sd: {item.difficultyBreakdown['Sedang']}</span>
                          <span>•</span>
                          <span className="text-red-400">Sl: {item.difficultyBreakdown['Sulit']}</span>
                        </div>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-center">
                    <button
                      ...
                      disabled={resettingId === item.song_id || (item.totalRatings === 0 && item.totalDifficultyVotes === 0)}
