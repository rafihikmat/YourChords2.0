...
const ChordCarousel: React.FC<ChordCarouselProps> = ({ chordName, initialPositionIndex = 0, className }) => {
    ...
    if (positions.length === 0) {
        return <div className="text-white">Chord not found</div>;
    }

    const currentPosition = positions[currentIndex];

    return (
        <div className={`flex flex-col items-center p-4 bg-slate-900/50 rounded-2xl border border-white/5 ${className}`}>
            {/* Diagram */}
