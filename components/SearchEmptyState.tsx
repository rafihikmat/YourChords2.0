...
export default function SearchEmptyState(
    { searchQuery }: SearchEmptyStateProps,
) {
    ...
    const handleRequestSong = async () => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                setRequested(true);
                setFeedbackMsg(
                    `Request Terkirim ke Admin! Permintaan "${searchQuery.trim()}" telah berhasil dicatat.`,
                );
            } else {
                setFeedbackMsg(
                    data.error || "Gagal mengirim permintaan ke admin.",
                );
            }
        } catch (err: any) {
            setFeedbackMsg(err?.message || "Terjadi kesalahan koneksi.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center text-center py-16 px-6 max-w-xl mx-auto border border-white/10 rounded-3xl bg-surface/80 backdrop-blur-xl shadow-2xl relative overflow-hidden my-8">
            ...
            <p className="text-slate-300 text-xs md:text-sm leading-relaxed mb-6">
                Tidak ada chord yang cocok dengan pencarian{" "}
            ...
            <div className="flex flex-col items-center gap-3 w-full">
                    ...
                    {loading
                        ...
                        ? (
                            <>
                                ...
                                <span>Request Terkirim ke Admin!</span>
                            </>
                        )
                        : (
                            <>
                ...
                {feedbackMsg && (
                    <div
                        className={`mt-3 p-3.5 rounded-xl border text-xs font-bold flex items-center gap-2 w-full animate-fade-in ${
                            requested
                    ...
                    >
                        ...
                        <span className="text-left">{feedbackMsg}</span>
                    </div>
                )}
            </div>

            <p className="text-[11px] text-slate-500 mt-6 font-mono">
                💡 Tim Admin YourChords akan menerima permintaan lagu ini dan
                dapat menyedotnya langsung ke database.
            </p>
        </div>
    );
}
