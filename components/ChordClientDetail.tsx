...
export default function ChordClientDetail({ data }: { data: ChordData }) {
  ...
  const [selectedChordForDiagram, setSelectedChordForDiagram] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "Space") {
        e.preventDefault();
        setAutoScrollSpeed(
          (prev) => (prev > 0 ? 0 : lastActiveSpeedRef.current)
        );
  ...
  const totalTransposeSteps = transpose +
    calculateCapoTranspose(data?.title || "", capoFret);
    ...
    <div className="flex flex-col min-h-screen pb-40 animate-fade-in relative pt-20">
      ...
      <div className="print-header hidden print:block mb-6">
        ...
        <div className="text-xs text-gray-600 mt-2 flex flex-wrap gap-4 border-t border-gray-300 pt-2">
          <span>
            <strong>Key:</strong> {data.key || "Original"}
          </span>
          <span>
            <strong>Capo:</strong>{" "}
            {capoFret > 0 ? `Fret ${capoFret}` : "Tanpa Capo"}
          </span>
          <span>
          ...
          <span>
            <strong>Platform:</strong> YourChords
          </span>
        </div>
      </div>
        ...
        <div className="flex flex-col flex-1 pb-1">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <Sparkles
                className={`w-3.5 h-3.5 ${
                  isSimplified ? "text-white animate-pulse" : ""
                }`}
              />
            ...
            >
              ...
              <span>
                {showYouTubePlayer ? "Tutup YouTube" : "Putar Musik (YouTube)"}
              </span>
            </button>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-extrabold text-white">
                Catatan Pribadi & Strumming Pattern
              </h3>
            </div>
          ...
          <div className="no-print mb-4 bg-primary/15 border border-primary/40 text-primary-light rounded-xl p-3.5 px-5 flex items-center gap-3 backdrop-blur-md shadow-[0_0_20px_rgba(168,85,247,0.25)] animate-fade-in">
            ...
            <div className="text-xs md:text-sm">
              ...
              <span className="font-bold text-primary underline">
                Capo di Fret {capoFret}
              </span>{" "}
              pada gitar Anda untuk memainkan nada asli lagu ini!
            </div>
          </div>
        )}
          ...
          >
            {processedLines.map((line, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="song-section-block lyric-line min-h-[1.5em]"
                >
            ...
            <div className="flex items-center gap-3 border-b border-white/10 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Panduan belajar dan kunci gitar untuk lagu ini
                </p>
              </div>
            </div>
                    ...
                    <img
                      src={tutorial.thumbnail_url ||
                        `https://img.youtube.com/vi/${
                          extractYouTubeId(tutorial.video_id) ||
                          tutorial.video_id
                        }/hqdefault.jpg`}
                      alt={tutorial.title}
      ...
      <div className="no-print floating-controls fixed bottom-5 left-1/2 -translate-x-1/2 w-[94%] md:w-auto min-w-[320px] max-w-3xl bg-slate-950/90 backdrop-blur-2xl border border-white/15 rounded-2xl px-5 py-3.5 shadow-[0_10px_50px_rgba(0,0,0,0.95)] z-50 flex items-center justify-between gap-3 md:gap-5 transition-all">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <div className="flex items-center gap-1 bg-white/5 rounded-lg px-1 py-0.5 border border-white/10">
            <button
              ...
              className="p-1.5 hover:bg-white/10 rounded-md text-white transition-colors cursor-pointer"
            >
              <Minus className="w-3 h-3" />
            </button>
            <span className="text-white font-mono font-bold w-5 text-center text-xs">
              {fontSize}
            </span>
            <button
              onClick={() => adjustFontSize(2)}
        ...
        {/* Tool: Transpose */}
        <div className="flex flex-col items-center gap-1">
          <span className="text-[9px] text-slate-400 font-bold tracking-[0.15em] uppercase">
            Nada
          </span>
          <div className="flex items-center gap-1 bg-primary/10 border border-primary/30 rounded-lg px-1 py-0.5">
            <button
              onClick={() => handleTranspose(-1)}
              className="p-1.5 hover:bg-primary/20 rounded-md text-primary transition-colors cursor-pointer"
            >
              <Minus className="w-3 h-3" />
            </button>
        ...
        <div className="flex flex-col items-center md:items-start gap-1 flex-1 md:flex-none min-w-[150px] max-w-[210px]">
          <div className="flex items-center justify-between w-full">
            ...
            <span
              className={`text-[10px] font-mono font-extrabold px-1.5 py-0.5 rounded transition-all ${
                autoScrollSpeed > 0
          ...
          <div className="flex items-center gap-2 w-full">
            <button
              onClick={() =>
                setAutoScrollSpeed((prev) =>
                  prev > 0 ? 0 : lastActiveSpeedRef.current
                )}
              ...
              title={autoScrollSpeed > 0
                ? "Jeda Auto-Scroll (Spacebar)"
                : "Mulai Auto-Scroll (Spacebar)"}
            >
              {autoScrollSpeed > 0
          ...
          <div className="relative w-full max-w-md bg-surface border border-white/15 rounded-2xl p-6 shadow-[0_0_50px_rgba(168,85,247,0.3)] text-white">
            ...
            <div className="flex items-center gap-3 mb-4 border-b border-white/10 pb-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">
                  Pilih folder setlist atau buat baru
                </p>
              </div>
            </div>
              ...
              {userSetlists.length === 0
                ? (
                  <p className="text-xs text-slate-500 text-center py-4">
                    Belum ada setlist. Buat setlist baru di bawah.
                  </p>
                )
                : (
                  userSetlists.map((setlist) => {
                    const isAdded = addedSetlistIds.includes(setlist.id) ||
                      setlist.song_ids.includes(data.id);
                    return (
                      <div
                        key={setlist.id}
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            !isAdded && handleAddSongToSetlist(setlist.id)}
                          disabled={isAdded}
          ...
          <div className="relative w-full max-w-md bg-surface border border-white/15 rounded-2xl p-6 shadow-[0_0_50px_rgba(168,85,247,0.3)] text-white">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between items-center bg-white/5 p-2.5 rounded-lg border border-white/5">
                ...
                <kbd className="px-2.5 py-1 bg-primary/20 text-primary border border-primary/30 rounded font-mono font-bold text-xs">
                  Spacebar
                </kbd>
              </div>
              <div className="flex justify-between items-center bg-white/5 p-2.5 rounded-lg border border-white/5">
                <span className="text-slate-300">Kecepatan Scroll +/-</span>
                <kbd className="px-2.5 py-1 bg-white/10 text-white border border-white/20 rounded font-mono font-bold text-xs">
                  ↑ / ↓
                </kbd>
              </div>
              <div className="flex justify-between items-center bg-white/5 p-2.5 rounded-lg border border-white/5">
