...
import {
  getFallbackChordById,
  getFallbackChords,
  INITIAL_FALLBACK_CHORDS,
} from "./fallbackData";
...
export function normalizeSong(row: any): Song {
  ...
  const rawContent = row.content ||
    (typeof row.chords === "string"
      ? row.chords
      : row.chords
      ? JSON.stringify(row.chords)
      : "");
 ...
 */
export async function getUserSongNote(
  userId: string,
  songId: string,
): Promise<string> {
  if (!songId) return "";
...
export async function removeSongFromSetlist(
  ...
  if (targetIndex !== -1) {
    setlists[targetIndex].song_ids = setlists[targetIndex].song_ids.filter(
      (id) => id !== songId
    );
    setLocalSetlists(setlists);
  }
