...
export async function addSongToSetlist(
  ...
  try {
    let query = supabase.from('setlists').select('song_ids, user_id').eq('id', setlistId);
    ...
    if (!error && data) {
      const existingIds: string[] = Array.isArray(data.song_ids) ? data.song_ids : [];
      if (!existingIds.includes(songId)) {
        const updatedIds = [...existingIds, songId];
        let updateQuery = supabase.from('setlists').update({ song_ids: updatedIds }).eq('id', setlistId);
        if (userId && userId !== 'guest') {
    ...
    let altQuery = supabase.from('user_setlists').select('song_ids, user_id').eq('id', setlistId);
    if (userId && userId !== 'guest') {
    ...
    if (altData) {
      const existingIds: string[] = Array.isArray(altData.song_ids) ? altData.song_ids : [];
      if (!existingIds.includes(songId)) {
        const updatedIds = [...existingIds, songId];
        let updateAltQuery = supabase.from('user_setlists').update({ song_ids: updatedIds }).eq('id', setlistId);
        if (userId && userId !== 'guest') {
...
export async function deleteSetlist(setlistId: string, userId?: string): Promise<boolean> {
  ...
  try {
    let deleteQuery = supabase.from('setlists').delete().eq('id', setlistId);
    if (userId && userId !== 'guest') {
    ...
    let deleteAltQuery = supabase.from('user_setlists').delete().eq('id', setlistId);
    if (userId && userId !== 'guest') {
...
export async function reorderSetlistSongs(
  ...
  try {
    let updateQuery = supabase.from('setlists').update({ song_ids: songIds }).eq('id', setlistId);
    if (userId && userId !== 'guest') {
      updateQuery = updateQuery.eq('user_id', userId);
    }
    ...
    let altQuery = supabase.from('user_setlists').update({ song_ids: songIds }).eq('id', setlistId);
    if (userId && userId !== 'guest') {
      altQuery = altQuery.eq('user_id', userId);
    }
    await altQuery;
...
export async function getUserFavorites(userId: string): Promise<Song[]> {
  ...
  try {
    ...
    let songIds: string[] = (userFavs || []).map((f: any) => f.song_id).filter(Boolean);
    ...
    if (songFavs && songFavs.length > 0) {
      const extraIds = songFavs.map((f: any) => f.song_id).filter(Boolean);
