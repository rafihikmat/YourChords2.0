...
export async function saveUserSongNote(
    ...
    if (
        arg3 !== undefined &&
        (arg1 === "guest" || arg1 === "demo-user" ||
            (arg2.length < 50 && arg3.length > 50))
    ) {
        userId = arg1;
        songId = arg2;
        notesContent = arg3;
 ...
 */
export async function checkIsSongLiked(
    arg1: string,
    arg2?: string,
): Promise<boolean> {
    const { songId, userId } = parseUserAndSongIds(arg1, arg2);
        ...
        if (isLiked) {
            await Promise.all([
                supabase.from("user_favorites").delete().eq("user_id", userId)
                    .eq("song_id", songId),
                supabase.from("song_favorites").delete().eq("user_id", userId)
                    .eq("song_id", songId),
            ]);
            return false;
        } else {
            await Promise.all([
                ...
                supabase.from("song_favorites").insert({
                    user_id: userId,
                    song_id: songId,
                }),
            ]);
