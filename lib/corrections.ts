 ...
 */
export async function submitSongCorrection(params: {
    songId: string;
    userId: string | null;
    reason: string;
    ...
    try {
        const { songId, userId, reason, proposedContent, originalContent } =
            params;
        ...
        if (error) {
            ...
            return {
                success: false,
                message: error.message || "Gagal menyimpan usulan perbaikan.",
            };
        }
        ...
        return {
            success: true,
            message:
                "✨ Saran perbaikan berhasil dikirim! Terima kasih atas kontribusi Anda.",
        };
    } catch (err: any) {
        console.error("[SUBMIT CORRECTION EXCEPTION]:", err);
        return {
            success: false,
            message: err?.message ||
                "Terjadi kesalahan sistem saat mengirim usulan.",
        };
    }
}
...
export async function getPendingCorrections(): Promise<SongCorrection[]> {
    try {
        ...
        const songIds = Array.from(
            new Set(corrections.map((c: any) => c.song_id).filter(Boolean)),
        );
        const userIds = Array.from(
            new Set(corrections.map((c: any) => c.user_id).filter(Boolean)),
        );
...
export async function approveCorrection(
    ...
    try {
        ...
        if (songUpdateErr) {
            console.warn(
                "[APPROVE CORRECTION SONGS TABLE WARN]:",
                songUpdateErr.message,
            );
        ...
        return {
            success: true,
            message: "Perbaikan berhasil diterapkan ke lagu utama!",
        };
    } catch (err: any) {
        console.error("[APPROVE CORRECTION EXCEPTION]:", err);
        return {
            success: false,
            message: err?.message || "Gagal menerapkan perbaikan.",
        };
    }
}
...
export async function rejectCorrection(
    ...
    try {
        ...
        if (error) {
            ...
            return {
                success: false,
                message: "Gagal menolak usulan perbaikan.",
            };
        }
        ...
        return {
            success: false,
            message: err?.message || "Gagal menolak usulan.",
        };
    }
}
